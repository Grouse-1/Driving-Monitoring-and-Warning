...
import { GetSeller } from '@/api/getInfo/index';
import {Delete, Edit} from "@element-plus/icons";
import Dialog from './components/dialog.vue'
import {ElMessage, ElMessageBox} from 'element-plus'
import {delSeller} from "@/api/CRUD/seller.js";
...
const initMenuList = async() => {
  const res = await GetSeller.sellerInfo(queryForm.value)
  console.log(res)
...
const handleDialog=(row)=>{
  if(!row){
    dialogTitle.value = '添加商户信息'
    dialogTableValue.value = null
  }
  else {
    dialogTitle.value = '编辑商户信息'
...
<template>
  <el-card>
    ...
    <el-table :data="sellerList" stripe  style="width: 100%">
      <el-table-column prop="name" label="账号" width="100" />
      <el-table-column prop="password" label="密码" width="150" />
      <el-table-column prop="shopname" label="商店名称" width="200" />
